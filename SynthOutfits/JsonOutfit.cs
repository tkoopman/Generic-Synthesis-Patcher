...
namespace SynthOutfits
{
    ...
    public class JsonOutfit (string editorID)
    {
        ...
        /// <summary>
        ///     If items that are to be added to Outfit are missing, should the outfit be skipped?
        ///     - Any (Default): Skip if any item is missing.
        ///     - All: Only skip if all items are missing which would make an empty outfit.
        ///     - Never: Never skip the outfit.
        /// </summary>
        [JsonProperty]
        ...
        {
            if (Items.Count == 0 && SkipIfMissing != SkipIfMissing.Never)
        ...
        public Outfit? ToOutfit ()
        {
            ...
            foreach (string item in Items)
            {
                if (!Common.TryGetRecord<IOutfitTargetGetter>(item, Program.State.LinkCache, out var record))
                {
            ...
            if (SkipIfMissing != SkipIfMissing.Never && outfit.Items.Count == 0)
            {
